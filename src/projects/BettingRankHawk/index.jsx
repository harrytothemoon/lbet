// Hawk Betting Rank Main Component
...
import { hawkAPI, formatDate } from "./api";
...
const BettingRankHawk = () => {
  ...
  const loadWeekRankings = useCallback(
    async (weekNumber, forceRefresh = false) => {
      const cacheKey = `hawk_sheet_week_${weekNumber}`;
      const cacheTimeKey = `hawk_sheet_time_week_${weekNumber}`;
  ...
  const loadTotalRanking = useCallback(
    async (forceRefresh = false) => {
      const cacheKey = "hawk_total_ranking";
      const cacheTimeKey = "hawk_total_ranking_time";
      ...
      try {
        ...
        for (let week = 1; week <= CONFIG.ACTIVITY.totalWeeks; week++) {
          ...
          if (gid !== undefined) {
            ...
            const weekCacheKey = `hawk_sheet_week_${week}`;
            const weekCacheTimeKey = `hawk_sheet_time_week_${week}`;
            let weekRankings;
  ...
  const handlePlayerQuery = async (playerId, weekToQuery = currentWeek) => {
    ...
    try {
      ...
      if (isCurrentWeek) {
        ...
        const cacheKey = `hawk_sheet_week_${weekToQuery}`;
        const cacheTimeKey = `hawk_sheet_time_week_${weekToQuery}`;
        ...
        const now = new Date();
        // Format: YYYY-MM-DD HH:MM (Hawk API format)
        const endTime = formatDate(now).substring(0, 16); // Remove seconds
        const startTime = currentWeekPeriod.start.substring(0, 16);

        // Check cache for current week player API data (5 minutes cache)
        const playerApiCacheKey = `hawk_api_${playerId}_${weekToQuery}`;
        const playerApiCacheTimeKey = `hawk_api_time_${playerId}_${weekToQuery}`;
        ...
        if (
          ...
          apiResult = await hawkAPI.getPlayerBetLogSummary(
            playerId,
            startTime,
            endTime
          );
        ...
        const currentWeekCumulativeCacheKey = `hawk_player_cumulative_current_${playerId}_${weekToQuery}`;
  ...
  const handleDialogWeekChange = async (newWeek) => {
    ...
    try {
      ...
      if (isCurrentWeek) {
        ...
        const cacheKey = `hawk_sheet_week_${newWeek}`;
        const cacheTimeKey = `hawk_sheet_time_week_${newWeek}`;
        ...
        // Check cache for current week player API data (5 minutes cache)
        const playerApiCacheKey = `hawk_api_${currentPlayerId}_${newWeek}`;
        const playerApiCacheTimeKey = `hawk_api_time_${currentPlayerId}_${newWeek}`;
        ...
        // Check cache for cumulative data first (shares time with API cache)
        const currentWeekCumulativeCacheKey = `hawk_player_cumulative_current_${currentPlayerId}_${newWeek}`;
        ...
        const cumulativeCacheKey = `hawk_player_cumulative_${currentPlayerId}_${newWeek}`;
  ...
  const [activeMascot] = useState(() =>
    Math.floor(Math.random() * CONFIG.SITE.mascots.length)
  );
  ...
  return (
    ...
    >
      ...
      {/* Mascot 吉祥物 */}
      <img
        src={CONFIG.SITE.mascots[activeMascot]}
        alt="Mascot"
        className="fixed bottom-0 right-0 w-48 md:w-64 lg:w-80 opacity-40 pointer-events-none z-10 animate-bounce-slow"
        style={{
...
export default BettingRankHawk;
